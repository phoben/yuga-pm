...
了解业务背景是理解需求的基础。**使用 AskUserQuestion 工具逐一提问**：
...
```json
{
  "questions": [{
    ...
    "multiSelect": false,
    "options": [
      { "label": "从零开始(推荐)", "description": "全新项目，还没有任何基础" },
      { "label": "已有原型", "description": "有初步原型或演示版本，需要完善" },
      { "label": "已有系统", "description": "有运行中的系统，需要优化或扩展" },
      { "label": "系统重构", "description": "现有系统需要大规模改造或替换" }
    ]
  }]
}
```
...
**必须使用 AskUserQuestion 工具进行提问**，而非直接输出文本问题。这确保用户可以通过点击选项快速响应。
...
| **提供选项** | 为每个问题提供 2-4 个预设选项，降低用户回答成本 |
| **保留"其他"** | AskUserQuestion 自动提供"其他"选项，允许用户自定义回答 |
| **单选为主** | 大多数问题使用单选（multiSelect: false） |
| **多选适度** | 仅当需要收集多个独立选择时使用多选（multiSelect: true） |

### 单选 vs 多选判断

```
单选场景（multiSelect: false）：
- 业务领域选择
- 优先级判断
- 技术方案决策
- 是/否确认
...
{
  "questions": [{
    "question": "这个项目主要服务于哪个业务领域？",
    "header": "业务领域",
    "multiSelect": false,
    "options": [
      { "label": "电商零售", "description": "在线购物、订单管理、库存管理" },
      { "label": "企业协作", "description": "办公自动化、项目管理、团队协作" },
      { "label": "数据服务", "description": "数据分析、报表、BI工具" },
      { "label": "客户管理", "description": "CRM、客户服务、营销自动化" }
    ]
  }]
}
```
...
{
  "questions": [{
    "question": "这个系统的目标用户群体包括哪些？",
    "header": "目标用户",
    "multiSelect": true,
    "options": [
      { "label": "内部员工", "description": "公司内部使用系统的员工" },
      { "label": "终端客户", "description": "使用产品的最终用户" },
      { "label": "合作伙伴", "description": "供应商、经销商等合作方" },
      { "label": "运营管理员", "description": "负责系统运维和管理的人员" }
    ]
  }]
}
```
...
{
  "questions": [{
    "question": "这个项目的核心目标是什么？",
    "header": "核心目标",
    "multiSelect": false,
    "options": [
      { "label": "效率提升", "description": "主要解决效率低下、人工操作繁琐的问题" },
      { "label": "业务拓展", "description": "支持新业务场景或新市场拓展" },
      { "label": "成本控制", "description": "降低运营成本、减少资源浪费" },
      { "label": "合规风控", "description": "满足监管要求、降低业务风险" }
    ]
  }]
}
```
...
```
1. 根据当前探索阶段，准备一个明确的问题
2. 设计 2-4 个预设选项（每个选项需有 label 和 description）
3. 调用 AskUserQuestion 工具
4. 等待用户响应
5. 根据用户回答，决定下一个问题或进入下一阶段
```

### 禁止的行为

- ❌ 一次性提出多个问题
- ❌ 直接输出文本问题而不使用 AskUserQuestion
- ❌ 提供过多选项（最多 4 个）
- ❌ 选项描述模糊或不相关
...
| 原则 | 说明 |
|-----|------|
| **一次一问** | 使用 AskUserQuestion 每次只提出一个问题 |
| **选项优先** | 为每个问题提供 2-4 个预设选项，降低回答成本 |

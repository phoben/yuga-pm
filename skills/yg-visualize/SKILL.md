...
| 方式 | 说明 | 推荐 |
|------|------|------|
| 传递完整内容 | 主 Agent 读取 → 传给 SubAgent → 返回结果 | 禁止 |
| 传递路径 | 主 Agent 只传路径 → SubAgent 自行读写 | 必须 |
...
| 阶段 | 主要步骤 | 输出 |
|------|----------|------|
| **阶段1** 文档预处理 | 创建任务列表 → 确定路径 → 执行脚本获取 outline → 校验规范 | JSON outline |
| **阶段2** 框架生成 | 复制资源文件 → 读取模板 → 填充 H1/元信息 → 生成章节骨架 → 打开预览 | 框架 HTML |
| **阶段3** 章节编写 | 逐个启动 SubAgent 处理 H2 章节（跳过"目录"） | 完整 HTML |
| **阶段4** 质量验证 | 检查占位符残留 → HTML 格式验证 → 输出最终文件 | 最终 HTML |
...
```
TodoWrite(更新主任务状态：阶段1-2 completed，阶段3 in_progress)
```

### 步骤2: 动态创建章节子任务

**为每个 H2 章节创建 TodoWrite 任务（跳过"目录"章节，见"关键规则 > 跳过规则"）：**
...
for section in outline.sections:
    if section.text in ["目录", "目录导航", "Table of Contents"]:
        continue
    todos.append({
...
**逐个处理章节（跳过"目录"章节），每完成一个就更新 TodoWrite：**

```
for section in outline.sections:
    # 跳过"目录"章节（见"关键规则 > 跳过规则"）
    if section.text in ["目录", "目录导航", "Table of Contents"]:
        continue
    ...
    Agent({
        ...
        prompt: `
遵循 ${CLAUDE_SKILL_DIR}/agents/fill-section.md 规范。

**任务参数：**
- 源文档: {source_doc_path}
- 目标 HTML: {output_html_path}
- 章节: {section.id} "{section.text}"
- 行号: {section.startLine}-{section.endLine}

**关键要求：** 使用 Grep + offset/limit 精准读写，禁止全量读取。完成后报告状态。
        `,
...
TodoWrite 自动显示可视化进度，章节任务逐个标记 completed。
...
**推荐：** 使用 VS Code Live Server 自动刷新。

详细方案见：`${CLAUDE_SKILL_DIR}/references/progress-perception.md`

---

## PDF导出功能

模板内置 PDF 导出按钮（顶栏右侧），支持一键导出带封面和目录的 PDF。

**填充区域：**

| data-fill | 内容 |
|-----------|------|
| `pdf-cover-title` | 文档标题 |
| `pdf-cover-project` | 项目名称 |
| `pdf-cover-version` | 版本号 |
| `pdf-cover-date` | 日期 |
| `pdf-toc-content` | H2/H3 目录列表 |

（"目录"章节跳过规则见"关键规则 > 跳过规则"）
...
标题为"目录"、"目录导航"、"Table of Contents"的 H2 章节：
- 不生成占位符
- 不创建任务
- 不添加导航项
- 不生成 PDF 目录项
...
| 阶段 | 主要步骤 | 输出 |
|------|----------|------|
| **阶段1** 问题解析 | 确定 HTML 路径 → 解析自然语言定位章节 → 判断问题类型 | 问题定位 |
| **阶段2** 方案规划 | 逐章节精准读取 → 分析问题 → 汇总修复方案 → 用户确认 | 修复方案 |
| **阶段3** 批量执行 | 创建任务 → 逐个调用 SubAgent 执行修复 → 输出摘要 | 修复完成 |
...
for 修复任务 in 修复方案列表:
    ...
    Agent({
        ...
        prompt: `
遵循 ${CLAUDE_SKILL_DIR}/agents/tweak-section.md 规范。
...
**关键要求：** Grep + offset/limit 精准定位，Edit 最小化修改。只报告状态。
        `
    })
